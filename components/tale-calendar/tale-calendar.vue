				...
				<template v-for="item in visibleDays" :Key="item.day">
					<view class="day-box flex-column">
						...
						<template v-if="showDot">
							<text v-if="isCurrentMonth(item.day) && item.data.dot && item.data.active" class="day-dot"></text>
							<text v-if="isCurrentMonth(item.day) && item.data.dot && !item.data.active" class="day-dot dot-gray"></text>
						</template>
					</view>
				</template>
			</view>
